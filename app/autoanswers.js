...
class EdpuzzleAutoanswer {
  constructor(answers) { // Modified to accept answers
    this.answers = answers; // Store loaded answers
  ...
  postAnswers(remainingQuestions, attemptId, total) {
    ...
    for (let i=0; i<questionsPart.length; i++) {
      ...
      // Get answers from JSON file instead of API
      const savedAnswer = this.answers.find(a => a.id === question._id);
      
      content.answers.push({
        "questionId": question._id,
        "choices": savedAnswer ? savedAnswer.answers : [],
